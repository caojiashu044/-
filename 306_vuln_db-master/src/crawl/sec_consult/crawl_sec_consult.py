...
from crawl.dataProce import insert_mongo, init_item, getDeepin, isInDeepin


class Sec_consult(object):
    ...
    def getDetail(self,url):
        ...
        if res.status_code == 200:
            ...
            item = {}
            item['Name'] = str(b_name)
            item['Title'] = str(detail_title)
            item['Product'] = str(detail_product)
            item['Vulnerable_Version'] = str(detail_vulnerable_version)
            item['Fixed_Product'] = str(detail_fixed_product)
            item['Cve_Number'] = str(detail_cve_number)
            item['Impact'] = str(detail_impact)
            item['Homepage'] = str(detail_homepage)
            item['Found'] = str(detail_found)
            item['By'] = str(detail_by)
            item['teaser'] = str(detail_teaser)

            item['Vendor_Description'] = str(vendor_description)
            item['Business_recommendation'] = str(business_recommendation)
            # Business_recommendation = scrapy.Field()
            item['Vulnerability_Overview'] = str(vulnerability_overview)
            item['Proof_Of_Concept'] = str(proof_of_concept)
            item['Tested_Versions'] = str(tested_versions)

            item['Vendor_Contact_Timeline'] = str(vendor_contact_timeline)

            item['Solution'] = str(section_solution)
            item['Workaround'] = str(section_workaround)
            item['Advisory_URL'] = str(section_advisory_url)
...
if __name__ == '__main__':
    ...
    system = db['system']
    agent = Sec_consult('sec_consult', collection, 'Name', system)
