...
struct FCB			//定义文件目录项FCB的结构(共16个字节)
{
	...
	char Fattrib;				//文件属性 详见手册
	short int Addr;				//文件首块号 0代表空文件 文件名的第一个字符串ascii码为0E5H或00H代表空目录项，前者是被删除后者是从来没用过
	short int Fsize;			//文件长度
};
// 目录中的第一个目录项存储父目录的FCB(该目录项的名字为“..”)，其地址是父目录的首块号、
// 根目录存储在Disk[1]～Disk[30]的盘快中
...
// 用户打开文件表用数组UOF uof[S]表示，其中S=32，即最多允许同时打开32个文件。
...
int BatchComd(void);
int FAT[K];						//FAT表,盘块数为K 系统重新读入内存的时候FAT[0]表示闲置的盘块数
...
int main()
{
	...
	FCB *fcbp;
	// *********** 初始化FAT和Disk ************
	for (i=0;i<K;i++)	//开始时所有盘块空闲 5000个磁盘快
		FAT[i]=0;		//空闲盘块标记
	...
	//读入磁盘块Disk[ ]信息
	ffi.open("Disk2008.dat",ios::binary);
	if (!ffi)
	{
	...
	ffi.open("UdTab2008.dat",ios::binary );
	if (!ffi)
	{
...
void ExecComd(int k)		//执行命令
{
	...
	char CmdTab[][COMMAND_LEN]={"create","open","write","read","close",
		...
		"undel","Prompt","udtab","fc","replace","temppath","batch"};
...
int BatchComd() {
    ...
    if (s == 0){
        ...
        while (s > 0) {
            ...
            if (i < SIZE) // 这个盘快里读完了 但是文件内容还没读完 就读取下一个盘快
                break;
            s = FAT[s];
        }
        Buffer[jj] = '\0';
