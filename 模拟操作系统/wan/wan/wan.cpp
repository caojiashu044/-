...
#include <cstring>		//strcpy(),_stricmp()
...
// 假设根目录的首块号是1
const short int rootpath_fblock = 1;

// 初始化rootpath
CurPath rootpath = { rootpath_fblock, "/", rootpath_fblock };
...
int FcComd(int);                 //对两个文件进行比较
int ReplaceComd(int);           //新增replace命令，实现文件取代
void BatchComd(int);            //新增batch命令，实现批处理
...
void main()
{
	...
	strcpy_s(curpath.cpath, sizeof(curpath.cpath), "/"); 		//根目录的路径字符串
...
// 读入文件分配表FAT
char yn;
std::ifstream ffi("FAT2008.txt");
if (!ffi) {
	std::cout << "Can't open FAT2008.txt!\n";
	std::cin >> yn;
	exit(0);
}
for (int i = 0; i < K; i++) { // 从文件FAT2008.txt读入文件分配表FAT
	if (ffi >> FAT[i]) {
		// 成功读取
	}
	else {
...
// 读入磁盘块Disk[ ]信息
ffi.open("Disk2008.dat", std::ios::binary);
if (!ffi) {
	std::cout << "Can't open Disk2008.dat!\n";
	std::cin >> yn;
	exit(0);
}
for (int i = 0; i < K; i++) { // 从文件Disk2008.dat读入盘块内容
	if (!ffi.read((char*)&Disk[i], SIZE)) {
		break; // 读取失败
	}
}
ffi.close();
...
ffi.open("UdTab2008.dat", std::ios::binary);
if (!ffi) {
	std::cout << "Can't open UdTab2008.dat!\n";
	std::cin >> yn;
	exit(0);
}
for (int i = 0; i < DM; i++) { // 从文件UdTab2008.dat读入盘块内容
	if (!ffi.read((char*)&udtab[i], sizeof(udtab[0]))) {
		break; // 读取失败
	}
}
ffi.close();
...
int ProcessPath(char* path, char*& Name, int k, int n, char attrib)
{
	...
	{
		s = curpath.fblock;
		strcpy_s(temppath, sizeof(temppath), curpath.cpath);
	}
	else
	{
		if (i == 0)
		{
			s = 1;
			strcpy_s(temppath, sizeof(temppath), "/");
		}
		else
		{
			path[i] = '\0';
...
int FcComd(int k) {
	...
	FCB* fcbp1;
	FCB* fcbp2;
	int s1 = FindFCB(fileName1, curpath.fblock, '\0', fcbp1);
	int s2 = FindFCB(fileName2, curpath.fblock, '\0', fcbp2);
	if (s1 < 0 || s2 < 0) {
		cout << "\n文件不存在。\n";
		return -2;
	}
	...
	if (pos == fcbp1->Fsize && pos == fcbp2->Fsize) {
		...
		if (pos < fcbp1->Fsize && pos < fcbp2->Fsize) {
			cout << "\n文件不同，在位置 " << pos + 1 << " 处：";
			cout << "文件1内容是 " << buffer1[pos] << "，文件2内容是 " << buffer2[pos] << endl;
		}
...
void ExecComd(int k)		//执行命令
{
	...
	char CmdTab[][COMMAND_LEN] = { "create","open","write","read","close",
		...
		"undel","Prompt","udtab" ,"fc","replace"};
	int M = sizeof(CmdTab) / COMMAND_LEN;	//统计命令个数
	for (cid = 0; cid < M; cid++) {			//在命令表中检索命令
		if (_stricmp(CmdTab[cid], comd[0]) == 0)//命令不区分大小写
			break;
	}
	int i_uof = Check_UOF(comd[1]);  // 查找文件是否已打开
	if (i_uof != S && (cid == 0 || cid == 1 || cid == 2 || cid == 3 || cid == 12)) {
		pre_i = i_uof;  // 更新pre_i为当前操作文件的索引
	}
	...
	case 25:UdTabComd();		//udtab命令
		break;
	case 26: FcComd(k);          // fc命令
		break; 
	case 27: ReplaceComd(k);    // 处理 replace 命令
		break; 
	default:cout << "\n命令错:" << comd[0] << endl;
	}
}
...
void HelpComd() {
	const int numCommands = 28; // 更新命令数量，加入replace命令后的数量
	const char* commands[numCommands] = {
		"create", "open", "write", "read", "close",
		"del", "dir", "cd", "md", "rd", "ren",
		"attrib", "copy", "type", "rewind", "fseek",
		"block", "closeall", "uof", "undel", "exit",
		"prompt", "fat", "check", "rewind", "fseek",
		"fc", "replace" 
	};
	const char* commandDetails[numCommands] = {
		"创建新文件：<文件名>[ <文件属性>]\n  - 文件属性是r（只读）、h（隐藏）或s（系统）。",
		"打开文件：<文件名>\n  - 打开已存在的文件以便操作。",
		"在指定位置写文件：<文件名> [<位置/app>[ insert]]\n  - 在文件的第n个字节处开始写入，可以是插入或覆盖模式。",
		"读文件：<文件名> [<位置m> [<字节数n>]]\n  - 从第m字节处读n个字节，若无参数则读至文件末尾。",
		"关闭文件：<文件名>\n  - 关闭已打开的文件。",
		"删除文件：<文件名>\n  - 删除指定的文件。",
		"显示当前目录：<路径名> [|<属性>]\n  - 显示目录内容，可指定显示隐藏或系统文件。",
		"改变当前目录：<路径名>\n  - 切换到指定的目录。",
		...
		"删除指定目录：<路径名>\n  - 删除空目录，若目录非空则失败。",
		"文件更名：<旧文件名> <新文件名>\n  - 将文件从旧名改为新名。",
		"修改文件属性：<文件名> [±<属性>]\n  - 修改文件的属性，如设置为只读、隐藏或系统文件。",
		"复制文件：<源文件名> [<目标文件名>]\n  - 将文件从源路径复制到目标路径。",
		"显示文件内容：<文件名>\n  - 显示文件的内容。",
		"将读、写指针移到文件开头：<文件名>\n  - 重置文件指针到文件开始位置。",
		"将读、写指针都移到指定位置：<文件名> <位置>\n  - 移动文件指针到指定的字节位置。",
		"显示文件占用的盘块号：<文件名>\n  - 显示文件所占用的磁盘块编号。",
		"关闭所有打开的文件：\n  - 关闭所有当前用户打开的文件。",
		"显示UOF(用户打开文件表)：\n  - 显示当前用户打开的所有文件的信息。",
		"恢复指定目录中被删除的文件：<路径名>\n  - 恢复在指定目录中被删除的文件。",
		...
		"显示FAT表中空闲盘块数(0的个数)：\n  - 显示当前磁盘剩余空闲块数。",
		"核对后显示FAT表中空闲盘块数：\n  - 核对并显示FAT表中空闲盘块数。",
		"移动文件指针到文件开头并清空内容：<文件名>\n  - 将文件指针移动到文件开头并清空文件内容。",
		"移动文件指针到指定位置：<文件名> <位置>\n  - 移动文件指针到指定的字节位置。",
		"比较两个文件：<文件名1> <文件名2>\n  - 若相同，显示“文件内容相同”字样；若不同，显示第一个不同字节的位置和各自的内容。若文件不存在则报错。",
...
int GetAttrib(char* str, char& attrib)
{
	...
	len = strlen(str);
	_strlwr_s(str, len + 1); // 使用 _strlwr_s 替换 _strlwr
	for (i = 1; i < len; i++)
	{
		switch (str[i])
		{
		case 'r': attrib = attrib | ar; break;
		case 'h': attrib = attrib | ah; break;
		case 's': attrib = attrib | as; break;
		default: cout << "\n命令中属性参数错误。\n"; return -1;
		}
	}
	return 1;
...
int CdComd(int k)
{
	...
	else		//命令有一个参数，将指定目录作为当前目录
	{
		...
		curpath.fblock = s;
		strcpy_s(curpath.cpath, sizeof(curpath.cpath), temppath);
		if (!dspath)
...
int M_NewDir(char* Name, FCB* p, short fs, char attrib)	//在p位置创建一新子目录
{
	...
	if (b < 0)
		return b;
	strcpy_s(p->FileName, sizeof(p->FileName), Name);	//目录名
	...
	q = (FCB*)Disk[b];
	strcpy_s(q->FileName, sizeof(q->FileName), "..");	//新目录中的第一个目录项名是“..”
...
int TypeComd(int k) {
	...
	char attrib = '\0';
	FCB* fcbp;
	const char* fileName = (k == 0) ? "NO_FILE" : comd[1]; // 如果没有参数，则使用NO_FILE作为标记
	...
	i_uof = pre_i; // 使用全局变量pre_i作为当前操作文件的索引

	// 检查文件是否打开
	if (uof[i_uof].state != 2) { // 状态为2表示文件已打开
		cout << "文件未打开或不存在，不能显示内容。\n";
		return -1;
	}

	// 显示文件内容
	char Buffer[SIZE + 1];
	int len = file_to_buffer(uof[i_uof].fp, Buffer); // 读取文件内容到Buffer
	if (len > 0) {
		Buffer[len] = '\0'; // 确保字符串以NULL结尾
		cout << Buffer << endl; // 显示文件内容
	}
	else {
		cout << "文件内容为空。\n";
	}
...
{
	strcpy_s(uof[i].fname, sizeof(uof[i].fname), gFileName);	//复制文件全路径名
...
int CreateComd(int k) {
	...
	strcpy_s(gFileName, sizeof(gFileName), temppath);
	i = strlen(temppath);
	if (temppath[i - 1] != '/')
		strcat_s(gFileName, sizeof(gFileName), "/");
	strcat_s(gFileName, sizeof(gFileName), FileName);  // 构造文件的全路径名
	...
	if (k == 2) {
		...
		while (*p != '\0') {  // 处理文件属性
			...
			ch = tolower(ch);
			switch (ch) {
			case 'r': attrib = attrib | (char)1; break;
			case 'h': attrib = attrib | (char)2; break;
			case 's': attrib = attrib | (char)4; break;
			default: cout << "\n输入的文件属性错误。\n"; return -3;
			}
			p++;
		}
	...
	strcpy_s(fcbp1->FileName, sizeof(fcbp1->FileName), FileName);  // 目录项中保存文件名
...
#define BSIZE 40*SIZE+1
	...
	strcpy_s(&buf[pos], sizeof(buf) - pos, Buffer);
...
int CloseComd(int k) {
	...
	if (k == 0) {
		...
		// 提供了文件名，查找并关闭文件
		FindPath(comd[1], attrib, 0, p);
		i_uof = Check_UOF(temppath);
	}
	else {
		cout << "\n命令参数个数错误。\n";
		return -1;
...
int PutUdtab(FCB* fp)
{
	...
	strcpy_s(udtab[Udelp].gpath, sizeof(udtab[Udelp].gpath), temppath);
	strcpy_s(udtab[Udelp].ufname, sizeof(udtab[Udelp].ufname), fp->FileName);
			...
			&& udtab[i].ufaddr == fdp->Addr)
		{
			...
			if (yn[0] == 'y' || yn[0] == 'Y')
			{
				...
				if (s >= 0)	//有重名文件
				{
					...
					strcpy_s(fdp->FileName, sizeof(fdp->FileName), Fname);
				}
				cc++;		//被恢复文件数增1
...
int CopyComd(int k) { //copy命令的处理函数：复制文件
	short int i, size, s01, s02, s1, s2, s22, b, b0, bnum, s0;
	...
	fcbp1 = fcbp; //记下源文件目录项指针
	strcpy_s(gFileName, sizeof(gFileName), temppath);
	i = strlen(temppath);
	if (temppath[i - 1] != '/')
		strcat_s(gFileName, sizeof(gFileName), "/");
	strcat_s(gFileName, sizeof(gFileName), FileName1); //构造文件的全路径名
	...
	if (k == 1) { //命令中无目标文件,同名复制到当前目录
		...
		if (strcmp(comd[2], "/") == 0 || strcmp(comd[2], "..") == 0) {
			// 处理复制到根目录或父目录的情况
			if (strcmp(comd[2], "/") == 0) {
				s02 = rootpath.fblock; // 根目录的首块号
			}
			else {
				if (curpath.parent == rootpath.fblock) {
					cout << "\n当前目录是根目录，没有父目录。\n";
					return -1;
				}
				s02 = curpath.parent; // 父目录的首块号
			}
			FileName2 = FileName1; // 使用源文件名作为目标文件名
		}
		else {
	...
	if (s2 >= 0) { //存在同名目标文件或目录
		if (fcbp->Fattrib <= '\07') { //存在同名目标文件
			char yn;
			cout << "目标目录中已存在文件 '" << FileName2 << "'，是否覆盖？(y/n) ";
			cin >> yn;
			if (yn == 'Y' || yn == 'y') {
				// 删除已存在的文件
				while (s2 > 0) {
				...
				i = FindBlankFCB(s02, fcbp2); //寻找首块号为s02的目录中的空目录项
				if (i < 0) {
					cout << "\n复制文件失败。\n";
					return i;
				}
				*fcbp2 = *fcbp1; //源文件的目录项复制给目标文件
				strcpy_s(fcbp2->FileName, sizeof(fcbp2->FileName), FileName2); //写目标文件名
				b0 = 0;
	...
	else { //FileName2尚不存在，在s02为首块号的目录内复制目标文件
		s22 = s02;
		i = FindBlankFCB(s22, fcbp2); //寻找首块号为s22的目录中的空目录项
		if (i < 0) {
			cout << "\n复制文件失败。\n";
			return i;
		}
...
int RenComd(int k)	//ren命令的处理函数：文件改名
{
	...
	if (IsName(comd[2]))
	{
		...
		if (s < 0)			//不重名
		{
			strcpy_s(fp->FileName, FILENAME_LEN, comd[2]);
			return 1;		//正确返回
		}
		cout << "\n存在与新文件名同名的文件。\n";
		return -5;
...
int FindPath(char* pp, char attrib, int ffcb, FCB*& fcbp)
{
		...
		for (i = 0; i < PATH_LEN; i++, pp++)
		{
			...
			else
			{
				if (i > 0)
				{
					...
					strcpy_s(paths[j], sizeof(paths[j]), Name);
					j++;
				}
				else
					return -1;		//路径错误
	...
	for (i = 0; i < j; i++)
	{
		...
		else
		{
			...
			strcat_s(temppath, sizeof(temppath), paths[i]);
		}
		if (ffcb)
		{
...
int ParseCommand(char* p)	
{
	...
	if (comd[0][i] != '\0')			
	{
		if (k > 0)
			for (j = k; j > 0; j--)
		strcpy_s(comd[j + 1], sizeof(comd[j + 1]), comd[j]);
		strcpy_s(comd[1], sizeof(comd[1]), &comd[0][i]);
		comd[0][i] = '\0';
		k++;
	}
	return k;
